                    ...
                    <div class="overflow-x-auto">
                        ...
                        <table class="min-w-full bg-white dark:bg-gray-700 rounded-lg overflow-hidden">
                            <thead class="bg-gray-100 dark:bg-gray-600">
                                <tr>
                                    <th
                                        class="w-1/12 px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                        Title</th>
                                    <th
                                        class="w-1/12 px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                        Start Time</th>
                                ...
                                @foreach ($events as $event)
                                    <tr class="group hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            {{ $event->start_time->format('H:i - d M Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            {{ $event->end_time->format('H:i - d M Y') }}</td>
                                        <td class="px-6 py-4">
                                            <div class="truncate max-w-xs">{{ $event->location }}</div>
                                        </td>
                                        @can('admin')
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-3">
                                                <a href="{{ route('attendees.index', $event->id) }}"
                                                    class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300"
                                                    onclick="event.stopPropagation()" title="Attendees">
